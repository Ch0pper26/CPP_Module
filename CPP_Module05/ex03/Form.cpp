...
/*   Updated: 2022/12/10 16:33:10 by eblondee         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (_signed == false)
		throw Form::NotSignedException("This form isn't signed yet");
	else if (executor.getGrade() > _execute_grade)
		throw Form::GradeTooLowException("grade too low to execute this form");
	else
		this->action();
}
...
Form::NotSignedException::NotSignedException(std::string error)
{
	_error = error;
}
